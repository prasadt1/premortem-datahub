"""Adversarial fixtures — expected to FAIL until binder P0 lands (Jul 30–31).

These lock known classifier bugs from the 2026-07-27 seam review.
Do not "fix" them by weakening gold labels — fix classify.py / binder instead.
...
# Today's classifier matches on column *name* only → false HARD.
# Gold after binder: UNAFFECTED (subject order_history not referenced).
# ---------------------------------------------------------------------------
...
def test_cte_single_source_filter_should_be_hard():
    # Gold HARD: order_status reaches WHERE (mirrors eval q25).
    sql = """
...
def test_cte_single_source_projection_should_be_soft():
    # Gold SOFT: all order_status refs are projection-only (mirrors eval q07).
    sql = """
    WITH t AS (SELECT order_id, order_status FROM order_history)
...
# Mark module: these are the known-broken suite until binder lands.
pytestmark = pytest.mark.xfail(
    strict=True,
    reason="Pending binder P0 / SELECT* / CTE fixes (spec 2026-07-28); must fail until then",
)
